...
void SEAdenitaVisualModel::initAtoms(bool createIndex /*= true*/) {
	...
	materialDataAtom_ = ADNArray<SBNodeMaterial*>(nPositions);
	nodeDataAtom_ = ADNArray<SBNode*>(nPositions);
...
void SEAdenitaVisualModel::initNucleotidesAndSingleStrands(bool createIndex /* = true */) {
	...
	materialDataNt_ = ADNArray<SBNodeMaterial*>(nPositions);
	nodeDataNt_ = ADNArray<SBNode*>(nPositions);
...
void SEAdenitaVisualModel::initDoubleStrands(bool createIndex /*= true*/) {
	...
	materialDataDS_ = ADNArray<SBNodeMaterial*>(nPositions);
	nodeDataDS_ = ADNArray<SBNode*>(nPositions);
...
#if 1
	SBPointerIndexer<ADNConformation> conformations1D;
	SBPointerIndexer<ADNConformation> conformations2D;
...
void SEAdenitaVisualModel::prepareBallsToNucleotides(double iv) {
	...
	materialData_ = materialDataAtom_;
	nodeData_ = nodeDataAtom_;
...
void SEAdenitaVisualModel::prepareNucleotidesToSingleStrands(double iv) {
	...
	indices_ = indicesNt_;
	materialData_ = materialDataNt_;
	nodeData_ = nodeDataNt_;
...
void SEAdenitaVisualModel::prepareDoubleStrandsToObjects(double iv) {
	...
	colorsV_ = colorsVDS_;
	materialData_ = materialDataDS_;
	nodeData_ = nodeDataDS_;
	...
	if (order == 1) {
		...
		SB_FOR(auto part, parts) {
			...
			SB_FOR(SBPointer<ADNSingleStrand> ss, singleStrands) {
				...
				auto nucleotides = ss->GetNucleotides();
				SB_FOR(SBPointer<ADNNucleotide> nt, nucleotides) center += nt->GetPosition();
	...
	SB_FOR(auto part, parts) {
		...
		SB_FOR(SBPointer<ADNSingleStrand> singleStrand, singleStrands) {
			...
			SB_FOR(SBPointer<ADNNucleotide> nt, nucleotides) {
				...
				const SBPosition3 nucleotideBackbonePosition = nt->GetBackbonePosition();
				...
				materialDataNt_(index) = material;
				nodeDataNt_(index) = nt();
...
void SEAdenitaVisualModel::prepareDoubleStrands() {
	...
	materialDataDS_ = ADNArray<SBNodeMaterial*>(nPositionsDS_);
	nodeDataDS_ = ADNArray<SBNode*>(nPositionsDS_);
	SBNodeMaterial* material = getMaterial();
	...
	SB_FOR(auto part, parts) {
		...
		SB_FOR(auto doubleStrand, doubleStrands) {
			...
			SB_FOR(auto baseSegment, baseSegments) {
				...
				materialDataDS_(index) = material;
				nodeDataDS_(index) = baseSegment;
...
void SEAdenitaVisualModel::displayForDebugging(SBNode::RenderingPass renderingPass) {
	...
	if (config.debugOptions.display_nucleotide_basis) {
		...
		SB_FOR(auto pair, ntMap_) {
			...
			SBPointer<ADNNucleotide> nt = pair.first;
		...
		SB_FOR(SBPointer<ADNSingleStrand> ss, singleStrands) {
			...
			SB_FOR(SBPointer<ADNNucleotide> nt, nucleotides) {
				...
				SB_FOR(SBPointer<ADNAtom> a, atoms) {
					...
					materialDataAtom_(index) = material;
					nodeDataAtom_(index) = a();
