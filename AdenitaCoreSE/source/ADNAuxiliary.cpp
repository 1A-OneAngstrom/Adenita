...
char ADNAuxiliary::getBaseSymbol(const std::string& name) {
...
std::string ADNAuxiliary::SBVector3ToString(const SBVector3& vec) {
...
std::string ADNAuxiliary::UblasVectorToString(const ublas::vector<double>& vec) {
	std::string st_pos = "";
	if (vec.size() == 3) {
...
std::string ADNAuxiliary::VectorToString(const std::vector<int>& vec) {
	std::string st_pos = "";
	bool start = true;
	for (const auto& v : vec) {
		if (!start) {
...
SBVector3 ADNAuxiliary::StringToSBVector(const std::string& vec) {
	std::vector<std::string> stringArray;
	std::size_t pos = 0;
	std::size_t found;
...
ublas::vector<double> ADNAuxiliary::StringToUblasVector(const std::string& vec) {
	std::vector<std::string> stringArray;
	std::size_t pos = 0;
	std::size_t found;
	while ((found = vec.find(',', pos)) != std::string::npos) {
...
std::vector<int> ADNAuxiliary::StringToVector(const std::string& vec) {
	std::vector<std::string> stringArray;
	std::size_t pos = 0;
	std::size_t found;
	while ((found = vec.find(',', pos)) != std::string::npos) {
...
SBVector3 ADNAuxiliary::UblasVectorToSBVector(const ublas::vector<double>& pos) {
	SBVector3 sbPos;
...
ublas::vector<double> ADNAuxiliary::SBVectorToUblasVector(const SBVector3& v) {
...
SBPosition3 ADNAuxiliary::VectorToSBPosition(const std::vector<double>& v) {
...
SBVector3 ADNAuxiliary::VectorToSBVector(const std::vector<double>& v) {
...
bool ADNAuxiliary::ValidateSequence(const std::string& seq) {
	bool val = true;
