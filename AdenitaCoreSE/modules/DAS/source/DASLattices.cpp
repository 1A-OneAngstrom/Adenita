...
void DASLattice::CreateHoneycombLattice(int maxRows, int maxCols) {
	...
	mat_ = ublas::matrix<LatticeCell>(maxRows, maxCols);
	const double angle = 120.0;  // deg
	const double delt_y = edgeDistance_ * sin(ADNVectorMath::DegToRad(angle - 90.0)) * 0.5;
	const double delt_x = edgeDistance_ * cos(ADNVectorMath::DegToRad(angle - 90.0));
	const double offset = delt_y;
...
LatticeCell DASLattice::GetLatticeCell(unsigned int row, unsigned int column) {
	...
	if (row >= rSize || column >= cSize) {
		...
		std::string msg = "Lattice overflow. Probably wrong lattice type was selected.";
