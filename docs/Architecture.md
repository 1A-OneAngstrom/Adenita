...
`AdenitaCoreSE/CMakeLists.txt` builds one shared library target named `SEAdenitaCoreSE` and installs data, resources, documentation, and the compiled SAMSON element. The top-level `CMakeLists.txt` resolves SAMSON SDK, Qt, OpenGL, and Boost dependencies before adding `AdenitaCoreSE`.
