...
This page preserves the original Adenita quick-start documentation for historical reference. It may mention old SAMSON versions, old issue trackers, and UI labels that no longer match this maintained fork. For current source-level documentation, start with the repository [README](../README.md), [architecture overview](Architecture.md), [data graph notes](AdenitaDataGraph.md), and [frame synchronization notes](FrameSynchronization.md).

## Historical Quick-Start Guide

Adenita is a 3D visualization and modeling toolkit for the in silico design of static and dynamic DNA nanostructures. Based on a multiscale data model, it visualizes DNA-based structures at different abstraction levels and enables users to load and create DNA origami structures and combine them with proteins.
...
## Historical Developer Note

Adenita was initially developed by Elisa De Llano and Haichao Miao.

The SAMSON Team maintains this fork for current SAMSON SDK compatibility and issue fixes.

## Adenita Quick-Start Guide

This historical guide gets you started designing DNA nanostructures with Adenita.
...
Adenita SAMSON edition is a [SAMSON](https://www.samson-connect.net) plugin. The original SAMSON Edition was available for **SAMSON 0.7.0** and **SAMSON 0.8.5**. Current users should follow the SAMSON Documentation Center tutorial linked at the top of this page. After installing SAMSON, you can select Adenita from a variety of plugins. Adenita can be combined with any of them. If you are unfamiliar with SAMSON, check out the [SAMSON tutorials](https://documentation.samson-connect.net/).

If you encounter problems while working with this maintained fork, use this repository's issue workflow when available rather than the historical original issue tracker.
...
* <img src="images/saveSelection.png" alt="Save selection" width="25" height="25" /> The user can choose to save a component for later use in the custom format (.adnpart).
...
* <img src="images/export.png" alt="Export" width="25" height="25" /> Options to export as a CSV sequence file or in a format appropriate for oxDNA are available here.
...
* <img src="images/connectSS.png" alt="Connect" width="25" height="25" /> Merges single or double strands. If strand ends are selected, they will be connected in the appropriate direction (5' to 3'). If nucleotides that are not 5' or 3' are selected, the strands will be broken in order to reconnect them at chosen points. It is also possible to insert a new double or single strand along the connection.
...
* <img src="images/twister.png" alt="Twister" width="25" height="25" /> Remove the entire twist of a double strand locally to observe the single strands that compose it as parallel lines.
